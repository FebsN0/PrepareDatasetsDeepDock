...
#
# COPYRIGHT (C) 2022 CHEMICAL COMPUTING GROUP ULC ("CCG").
# ALL RIGHTS RESERVED.
#
# ACCESS TO AND USE OF THIS SOFTWARE IS GOVERNED BY A SOFTWARE LICENSE
# AGREEMENT WITH CCG OR AN AUTHORIZED DISTRIBUTOR OF CCG; THIS SOFTWARE
# MAY NOT BE USED WITHOUT SUCH A LICENSE.
#
...
# Automatically generated on Fri Oct 14 13:19:33 2022
# MOE 2020.0901 (Chemical Computing Group)

# VERSION hpc_run.sh     63164.2022.4.28.16.25.35   $MOE/lib/sh
# VERSION hpc.svl        25449.2022.1.9.22.5.34     $MOE/svl/msys
# VERSION confsrch.svl   11292.2021.12.2.13.20.46   $MOE/svl/run
# VERSION moe_init.sh    62928.2020.10.21.11.39.52  $MOE/lib/sh
# VERSION moe_u.sh       46695.2022.1.9.22.1.38     $MOE/lib/sh
...
# === PREAMBLE:==============================================================
#
# NOTE:  "-e" is needed to avoid reporting a job success in the presence of
...
[ -n "${LINENO-}" ] && PS4='+ $LINENO '	# in case -x is used
...
#   STEP_6: RESVARS: prepare resource spec env.vars used by payload preamble
#   STEP_7: PAYLOAD: portion of "run.sh" generated ad-hoc by each app

task_N=1	# #tasks (except the epilog)
task_N0=1	# last task# (0 if epilog)
task_JN=1	# #submission cmds (+1 if epilog)
task_parts=0	# #partitions for sngl.node graphs
task_max=0	# max.#concurrent jobs
task_name='MOE(ventralemedialepfc)(pgm8dk)csearch'
task_qsys='sh'
...
_ecode=; _elabel=; _emsg=			# error code reported by _err
...
_cleanup () {
	...
	_x="$_log_last/_${_ppfx-}err.txt"	# instead of stderr file
...
# ERROR_CODES: 2=bad param 3=startup/config 4=bug  5=glitch (wrong assumptions)
# Codes NOT used with _err: 0=success 1=job failure
# Note: failed moe_init.sh etc should exit 2 [=bad init params] (not exit 3)

_err () {
    set +x			# turn off debugging trace (if is was on)
    _ecode=$1; shift; _elabel=$1; shift		# $1=code, $2=label, $3-=msg
	...
	[ -n "${_emsg:-}" ] && printf '_emsg: %s\n' "$_emsg" >> "$_x"
	_print_stack					2>> "$_x"

	[ -n "${_emsg:-}" ] &&  {
	    _x="$_log_last/_${_ppfx-}ivalxx"
	    printf '_emsg: %s\n' "$_emsg" >> "$_x"
	    [ -n "${_log_sdir-}" ] && _x="$_log_sdir/_${_ppfx-}ivalxx" || _x=
	    [ -n "$_x" -a -w "$_x" ] &&  printf '_emsg: %s\n' "$_emsg" >> "$_x"
	}
    fi
    exit "$_ecode"
}
...
# _ivalxx is an extended version of _ivalx, useful for error diagnostics
...
# assumptions about state variables associated with run.log.
...
_validate_log_dir () {
    ...
    _x="${task_logdir-}"
    [ -z "$_x" ]		&& _err 3 "$1"_xt2 '$task_logdir not set'
    [ ! -d "$_x" ]		&& _err 3 "$1"_xd0 "$_x not a directory"
...
# This is to ensure that single-quoted items stay single-quoted when
# transmitted to the remote in the form of _ssh/ssh arguments.
...
# are catenated with a space in between them (some versions have a line ending).
...
# Note: requote sngl.quotes in $@ in order to preserve them on the remote.
...
# value in MB. The input is extracted directly from QSYS. Exit 4 (=bug) on err.
...
_newQueue=false
_newSys=false
_nameCwd=false		# on submission, set task_name="${$PWD##*/}"
...
if [ -n "$_qsysd" ]; then		# non-SH compute task
	...
	$task_submit &&		_err 2 E_cmd1 "Unexpected -submit -$task_jctl"
    fi
	...
	* )	_err 2 E_logmode1 "Unexpected -logmode $task_logmode" ;;
    ...
    [ -n "$_rhost" ] && { task_jctl='upload'; _upEnsure=true; }
				...
				_err 2 E_cmd2 'Missing command (e.g. -submit)'
fi
[ -z "${task_mode-}" ] &&	_err 4 E_nomode2B 'Unknown $task_mode'

[ -z "${task_logmode##*[012]*}" ] \
    || _err 2 E_logmode2 "Unexpected -logmode $task_logmode"

[ -z "${task_mode-}" ] &&	_err 4 E_nomode3B 'Unknown $task_mode'
...
case "${task_qsys:=sh}" in	# set to "sh" if empty
    slurm | sge | lsf | sh ) ;;
    *) _err 2 E_qsys0 "Unsupported value: -qsys $task_qsys (sh|slurm|sge|lsf)";;
esac

# Ensure that missing jobname OK only if local submit & -namecwd
# Note: If remote, jobdir is set to $task_name (=jobname)
...
if [ -n "${testing-}" ]; then
	...
	*)	_err 5 E_test0 "Bad value: \$testing=$testing" ;;
    esac
fi
...
[ -n "$_rhost" ] && task_rdir="$task_rdir/$task_name";	# rdir ++ jname
...
# _set_rsh_fn verifies this runfile's existence on the remote (_ssh) host
...
_set_rsh_fn () {
	...
	cat <<-EOF | _ssh "$_rsh"		# test [ -f fn ] remotely
	    ...
	    # [ -n "\${_x##*/*}" ] && _x=	# ignore if no "/" in the path
...
# Capture the flow-through options for a remote command.
...
_seq () {
	...
	command -v disown 2> /dev/null >&2 && disown
...
_slurm () {
	...
	# process maximum concurrent jobs in array
	...
	# Note: The result of "sbatch', "$_job" will contain the job ID,
	# optionally followed by ";"-separated cluster name, e.g.
	#   "318438;ccg". On error: "sbatch: error: ..."
	# We try to extract the number and test that it _is_ a number.
	...
	_job="$(sbatch -o "$_log.out" -e "$_log.err" \
	    ...
	    $task_qargs_QSYS --export=ALL "$task_sh" -name "$task_name" "$@")"
	# eval "$_var"="${_job%%;*}"	# save the job id
	_job="${_job%%;*}"		# remove cluster name, e.g. ";ccg"
	[ "1$_job" -eq "1$_job" 2>/dev/null ] || {	# if not a number
	    printf '$_job: %s\n' "$_job" >&2
	    _emsg='_job: '"$_job"
	    _err 3 E_slurm5 'SLURM failed submission'
	}
	eval "$_var"="$_job"		# save the job id
...
_sge () {
	...
	*)  if $_in; then _pe="$_pe $_word"
    ...
    echo qsub -o "'$_log.out'" -e "'$_log.err'" -S /bin/sh -terse \
	    -V -cwd -N "$task_name" -t $_tlist$_max $_queue $_dep $_res -b n \
	    "'$task_sh'" -name "'$task_name'" "$@"

	# Note: The result of "qsub', "$_job" will contain the job ID,
	# optionally followed by "."-separated array index info, e.g.
	#   "6942.1-3:1"	... when "-t 1-3" is used
	# We try to extract the number and test that it _is_ a number.
	...
	_job="$(qsub -o "$_log.out" -e "$_log.err" -S /bin/sh -terse \
	    -V -cwd -N "$task_name" -t $_tlist$_max $_queue $_dep -b n $_res \
	    "$task_sh" -name "$task_name" "$@")"
	_job="${_job%%.*}"		# remove array size info, e.g. ".1-3:1"
	[ "1$_job" -eq "1$_job" 2>/dev/null ] || {
	    printf '$_job: %s\n' "$_job" >&2
	    _emsg='_job: '"$_job"
	    [ -z "$_job" ] && _err 3 E_sge5a 'SGE empty job ID'
	    [ -z "${_job##*qsub:*\"-b\"*}" ] && _err 3 E_sge5b 'SGE "-b" arg'
	    _err 3 E_sge5c 'SGE failed submission'
	}
	eval "$_var"="$_job"		# save the job id
	_x=".${_ppfx}${_var}_${_job}"	# record submission
	touch "${_log_sdir:?}/$_x" "${_log_last:?}/$_x"
...
_lsf () {
	...
	# Note: The result of "bsub', "$_job" will contain the job ID
	# in angle brackets, e.g.
	#   "Job <824> is submitted to default queue <normal>."
	# We try to extract the number and test that it _is_ a number.
	...
	_job="$(
		...
		-env "all" $_queue$_dep$_res $task_qargs_QSYS \
		"/bin/sh '$task_sh' -name '$task_name' $_argv")"
	_job="$(_subs "$_job" "#*<" "%%>*")"	# extract "<jobid>" from stdout
	[ "1$_job" -eq "1$_job" 2>/dev/null ] || {
	    printf '$_job: %s\n' "$_job" >&2
	    _emsg='_job: '"$_job"
	    _err 3 E_lsf5 'LSF failed submission'
	}
	eval "$_var"="$_job"		# save the job id
	_x=".${_ppfx}${_var}_${_job}"	# record submission
	touch "${_log_sdir:?}/$_x" "${_log_last:?}/$_x"
...
if $task_submit; then		# init & submit
	...
	# Special: If "-namecwd", use the CWD (at the moment of submission)
	# as the job name ($task_name);
	...
	# Set ".T0_unk" marker to indicate job submission. If job finishes with
	# success/failure, ".T0_unk" will be replaced with ".T0_ok"/".T_err".
	...
	# Each job will also receive the full environment of this process.


    _ecode=0; _elabel="E_${task_qsys-}Calls"	# subs.xxxCalls
    if   [ "$task_qsys" = "sge"   ]; then :
	qstat -help >/dev/null 2>&1 || _err 2 E_qsys1 "SGE not installed"
	qstat -Q >/dev/null 2>&1 &&  _err 2 E_qsys2 "SGE not installed"
	_sge "J1" "1" "" "" -- "$@"
    elif [ "$task_qsys" = "slurm" ]; then :
	squeue -V >/dev/null 2>&1 || _err 2 E_qsys3 "SLURM not installed"
	_slurm "J1" "1" "" "" -- "$@"
    elif [ "$task_qsys" = "lsf"   ]; then :
	bjobs -V >/dev/null 2>&1 || _err 2 E_qsys4 "LSF not installed"
	_lsf "J1" "1" "" "" -- "$@"
...
# task_gpu	: gpu device has been allocated

# Environment variable used by CUDA API, set by the queueing system on the
# compute host, e.g. CUDA_VISIBLE_DEVICES=0,1.  (Applies only to nvidia.)

[ -n "${CUDA_VISIBLE_DEVICES:-}" ] && task_gpu=1 || task_gpu=0

# Environment variable CUDA_VISIBLE_DEVICES is used by the CUDA API,
# and should be set by queueing system.  (applies to nvidia devices only)
...
if   [ "$task_qsys" = 'sge'   ]; then
	...
	# Some SGE's set only SGE_HGR_gpu and not CUDA_VISIBLE_DEVICES.
	# Note: Use only the first word (single GPU)
	# Safety: tolerate (illegal) syntax "(<num> blah)"

    if [ "$task_gpu" = '0' ] && [ -n "${SGE_HGR_gpu:-}" ]; then
	_x="${SGE_HGR_gpu%%[ ]*}"	# first word only
	_x="${_x#[(]}"			# safety: tolerate illegal "("
	[ -n "${_x}" ] && { task_gpu=1; CUDA_VISIBLE_DEVICES="$_x"; }
    fi
...
MOEBATCH () {
    ...
    [ -n "${task_gpu-}" ] && [ "${task_gpu}" -eq 0 ] && _nogpu="-nogpu"
    _arg="-stdc -errexit 1 -notty $_nthreads ${_nogpu-}"
	...
	# If qsys specified, get hostlist & (will) prepare a hostfile.
	# If SH, run moebatch now

    task_mpu="run.log/${task_id:-x}.mpu"
    case "${task_qsys-}" in
	slurm)		# SLURM
	    [ -z "${SLURM_NTASKS-}" ]	&& task_mpu=	;;	# safety
	sge)		# SGE
	    [ -z "${NSLOTS-}" ]		&& task_mpu=	;;	# safety
	lsf)		# LSF
	    [ -z "${LSB_MCPU_HOSTS-}" ] && task_mpu=	;;	# safety
	*)		# SH
	    task_mpu=						# safety
	    "$MOE/bin/moebatch" $_arg "$@" && return 0		# 1 task
	...
	# LSB_MCPU_HOSTS: similar to PE_HOSTFILE, below.
	# awk line arg: $1=node name $2=#tasks for that node

    if   [ -z "$task_mpu" ]; then :		# safety (non-mpu)
    elif   [ "$task_qsys" = "lsf" ]; then
	echo "${LSB_MCPU_HOSTS-}" | \
	...
	# PE_HOSTFILE: 4 columns, each line defines one node:
	#   hostname #processes que.name processor_range(if multiproc.machine)
	# awk line arg: $1=hostname $2=#tasks for that node
	...
	# srun hostname: outputs a list of nodes, one hostname per task.
	# (hostnames repeated if >1 task/host); awk line arg: $1= hostname
	...
	echo "" > "$task_mpu"			# clear the task_mpu file
    fi
	...
	# Use the machine file ONLY if #nodes > 1.

    if [ "$_mpu_N" -gt 1 ]; then		# > 1 node
	_mpu='-mpu'				# use MPU
	echo '$eof' >> "$task_mpu"		# $task_mpu is the hostfile
    else					# = 1 node
...
if [ -f 'run_qm.sh' ]; then
    cwd=$(pwd);
    if [ "$task_qsys" != "sh" -a -n "$task_shost" ]; then
	qhost="-host $task_shost:\"${cwd%/*}\""
    fi
    if [ "$task_qsys" = 'sh' -a "$task_logmode" != '1' ]; then
	sh run_qm.sh -- "$@"
    else
	sh run_qm.sh -qsys "$task_qsys" $qhost \
	    -qid "$task_qid" -qname "$task_qname" \
	    -submit -name "${task_name:?}" -qargs "$task_qargs_QSYS" -- "$@";
    fi
fi
exit 0
#endif
...
//
//	File: $HOME/csearch/run.sh
//	Date: Fri Oct 14 13:19:33 2022
//	Creator: Conformational Search in MOE (2022.02)
//
// LICENSE:
//
...
// OUTPUT FILES:
//
// Output Database: csearch.mdb
//
// USAGE:
//    run ['run.sh', [infile: infile, outfile: outdb, qm: flag]]
//    moebatch -run run.sh -def -infile infile -outfile outdb -qm flag
//    sh run.sh [job options] -- -def -infile infile -outfile outdb -qm flag
...
local function main arg
    local outdb    = 'csearch.mdb';
    local indb     = '';
    local indbORIG = '$HOME';
    local useQM = 0;
    local defFF = 0;

	// If arg is null, look at the command-line arguments.
	// structure can be a file or an MDB.  Only one file is
	// supported at this time, but processing will allow for
	...
	local def, qm;
	local out1, out2, in1, in2;
	if MOE_VERSION > 2020.09 then
	    [def, out1, qm, in1] = ArgvPull [
		['-def',['-o','-outfile'],'-qm',['-i','-infile']],
		[0, 1, 1, 1]
	    ];
	    if anytrue out1 then outdb = out1; endif
	    if anytrue in1  then indb  = in1; endif
	else
	    [def, out1, out2, qm, in1, in2] = ArgvPull [
		['-def','-o','-outfile','-qm','-i','-infile'],
		[0, 1, 1, 1, 1, 1]
	    ];

		// -outfile and -infile take precedence

	    if anytrue out1 then outdb = out1; endif
	    if anytrue out2 then outdb = out2; endif

		// Currently support just one infile

	    if anytrue in1 then indb = in1; endif
	    if anytrue in2 then indb = in2; endif
	endif

	if anytrue def then defFF = 1; endif
	if notnull qm  then useQM = notnot tonum qm; endif

    else
	if anytrue arg.outfile then
	    outdb = arg.outfile;
	elseif anytrue arg.o then	// deprecated
	    outdb = arg.o;
	endif
	if anytrue arg.infile then
	    indb  = arg.infile;
	elseif anytrue arg.i then	// deprecated
	    indb  = arg.i;
	endif

	if notnull arg.def then defFF = notnot tonum arg.def; endif
	if notnull arg.qm  then useQM = notnot tonum arg.qm;  endif
    endif

    if length outdb > 1 then
	exit 'Only one output mdb supported';
    endif
    if length indb > 1 then
	exit 'Only one input file supported';
    endif

    if anytrue indb then
	if not freadable indb then
	    exit twrite ['{} is not readable', indb];
	endif

	if not (ftype indb == 'file') then
	    exit twrite ['{} is not a file', indb];
	endif
    endif

    local psys    = SystemPush [];
    local [opt, theFF, theParams] = recover_opt [];
    local idbname = [];
    if ftype 'input_mol.moe' == 'file' then
	Open 'input_mol.moe';
    endif
    if ftype 'input_mdb.mdb' == 'file' then
	idbname = 'input_mdb.mdb';
    endif
    if not defFF then

	    // Create cleanup task, to restore ff after calculations

	local oldff       = fenvname (pot_Info []).filename;
	local oldpotsetup = pot_Setup [];
    ...
    if anytrue indb then
	local data = freadb [indb, 'char', 512];
	if io_Is_MDB [data, 0, 'moe_mdb'] == 'moe_mdb' then
	    opt.infile = indb;	// use the database
	else
	    opt.infile = '';
	    Close [force: 1];
	    Open indb;		// use the file contents
	endif

    else
	opt.infile = idbname;	// use the encoded mdb (if present)
    endif;

    write ['\nStarting Conformational Search: {}\n', asctime []];
    write ['\n'];

    if second task_wfork [master: 'parent', idle: 0] == 'child' then
        call ['ConfSearch', opt];
        exit [];
    endif
    if useQM then
        opt.method          = 'Refine Only';
        opt.useQM           = 1;
        opt.dbview          = 0;
        opt.infile          = opt.outfile;
        opt.infield         = 'mol_moe';
        opt.infile_esel     = 0;
        opt.outfile         = tok_cat [
            fbase opt.outfile, '_qm.', fext opt.outfile
        ];
        opt.jobopt          = [['title','csearch']];
        opt.in_batch        = 1;
        opt.QMopt.keepfiles = 1;
...
,           keepfiles : 1
,           runInternal : 0
,           filename_input : '/tmp/mopac_34257/mopac.mop'
,           filename_output : '/tmp/mopac_34257/mopac.out'
,           filename_root : '/tmp/mopac_34257/mopac'
,           solvmodel : ''
        ]
    ];

    local theFF     = '$MOE/lib/Amber10EHT.ff.gz';
    local theParams = [
        strEnable : 1
,       angEnable : 1
,       stbEnable : 1
...
,       state2 : 1
    ...
    return [the_options, theFF, theParams];
endfunction

global function ConfSearch_RecoverBatch []
    return recover_opt [];
endfunction
