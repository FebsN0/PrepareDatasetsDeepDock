...
# Automatically generated on Fri Oct 14 17:07:12 2022
...
# VERSION dock_u.svl     43564.2022.5.2.16.7.48     $MOE/svl/calc.svl
...
task_name='MOE(ventralemedialepfc)(z6p8xr)_HOME_dockSites_newVersions_dock_sito1'
task_qsys='sh'
...
MOEBATCH -run "$0" -parts "$task_parts" -tid "$task_id" "$@" -exit
exit 0
#endif
...
//
//	File: run.sh
//	Date: Fri Oct 14 17:07:12 2022
//	Creator: Docking in MOE (2022.02)
//
// LICENSE:
//
...
//
// Receptor: rec.moe
//   Ligand: $HOME/dockSites_oldVersions/dockSite1/csearch.mdb
//
// OUTPUT FILES:
//
// output: dock.mdb
//
// USAGE:
//    In a terminal shell (Unix) or in a Command Prompt (Windows), moebatch can
//    be used with the CLI flags:
//    
//         -rec  file : Receptor filename
//         -site file : Site selection expression
//         -lig  file : Ligand filename
//         -ph4  file : Pharmacophore filter filename
//         -outfile file : Output filename
//         -ed   file : Electron Density filename
//         -resume    : Resume calculation
//         -force     : Overwrite any existing output files
//         -mpu  N    : Perform docking in parallel moebatch instances
//         -nthreads N: Number of CPU threads used by forcefield calculations
//    
//    Examples:
//    
//        moebatch -run run.sh -o output_database.mdb
//        moebatch -run run.sh -mpu 8 -nthreads 1
//    
//    The shell script can be used to submit to HPC. Extra supported options:
//         -parts N   : Partition into N independent docking tasks
//    
//    Examples:
//    
//        sh run.sh -help
//        sh run.sh -qsys slurm -host head:./dir -parts 64 -upload -submit
//        sh run.sh -qsys slurm -host head:./dir -parts 64 -submit -- -resume

global argv;
function ArgvPull, Dock;

    // main calls Dock with the saved arguments.
    // new_opt overrides the saved option vector except these
    // special tag values:
    //
    //	    rec		new receptor specification
    //	    lig		new ligand specification
    //	    site	new site specification
    //	    outfile	new output path specification
    //
    // These values can also be changed with the CLI flags
    //      -rec, -lig, -site, -outfile.

local function main new_opt
    ArgvReset ArgvExpand argv;

    local _rec, _lig, _site, _ph4, _outfile;
    local task_i, task_parts, _jobdir;

    if MOE_VERSION > 2020.09 then
	[_rec, _lig, _site, _ph4, _outfile] = app last ArgvPull [
	    ['-rec','-lig', '-site', '-ph4', ['-outfile', '-o']], 1
	];
	[task_i, task_parts, _jobdir] = app last ArgvPull [
	    [['-tid', '-task_i'], '-parts', '-jobdir'],  1
	];
    else
	local out1, out2;
	local ti1, ti2;

	[_rec, _lig, _site, _ph4, out1, out2] = app last ArgvPull [
	    ['-rec','-lig', '-site', '-ph4', '-o', '-outfile'], 1
	];
	[ti1, ti2, task_parts, _jobdir] = app last ArgvPull [
	    ['-task_i', '-tid', '-parts', '-jobdir'],  1
	];
	_outfile = out1;	// -outfile has precedence
	if anytrue out2 then _outfile = out2; endif

	task_i = ti1;		// -tid has precedence
	if anytrue ti2 then task_i = ti2; endif
    endif

	// Saved argument values.
    ...
    function first_notnull v = v(x_min not app length v);

    rec		= first_notnull [new_opt.rec, _rec, rec];
    lig		= first_notnull [new_opt.lig, _lig, lig];
    site	= first_notnull [new_opt.site, _site, site];
    ph4fn	= first_notnull [_ph4, ph4fn];
    outfn	= first_notnull [new_opt.outfile, _outfile, outfn];
    jobdir	= first_notnull [_jobdir, jobdir];

    new_opt = tagdiff [new_opt, ['rec','lig','site','outfile']];
	...
	run_mode: 'run',
	hpc_opt: [
	    title: '$HOME/dockSites_newVersions/dock_sito1',
	    jhash: 'z6p8xr'
	],
	create_mdb: 0,
	embed3d: 0,
	...
	silent: 0,
	randseed: 3931055,
	placement_fcn: 'dock_place_Proxy',
	scoring_fcn: 'dock_score_London_dG',
	scoring_opt: [
	    silent: 0
	],
	refine_fcn: 'dock_refine_ForcefieldInduced',
    ...
    opt		= tagcat [new_opt, opt];	// function call options

    if notnull task_i		then opt.task_i	    = tonum task_i; endif
    if notnull task_parts	then opt.task_parts = tonum task_parts; endif

    if isnull opt.task_i and add opt.task_parts > 1 then
	exit '\nDock: task number must be specified with partitioned jobs\n'
	     'In the moebatch call use either:\n'
	     '    -parts 1 (to run as a single job) OR\n'
	     '    -tid task_number (to run a specific part)\n';
    endif
	...
	vdwBuffer1: 0,
	vdwBuffer2: 0,
	eleScale14: 0.8333333333000000342,
	eleDielectric: 1,
	eleBuffer: 0,
	solDielectric: 80,
